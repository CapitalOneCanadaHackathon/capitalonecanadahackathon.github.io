/*
 * Modified from https://github.com/maraisr/mosaic.js
 * MODIFICATIONS:
 * In function Light() {...};
 * Adjusted the mouse coordinates to animate on the specific container
 * of the mosaic, instead of on the whole page.
 *
 */
...
var Light = function () {
    function Light(ray) {
        ...
        // Modification: Adjust mouse coordinates to animate on the specific container
        // of the mosaic, instead of on the whole page.
        var container = document.getElementById("hero-mosaic");
        var containerRect = document.getElementById("hero-mosaic").getBoundingClientRect();
        var containerLeftOffset = parseInt(window.getComputedStyle(container.parentNode, null).marginLeft)
        var containerTopOffset = parseInt(window.getComputedStyle(container.parentNode, null).marginTop)

        self.addEventListener('mousemove', function (e) {
            if ((e.x >= containerRect.left && e.x <= (containerRect.left + containerRect.width)) &&
                (e.y >= containerRect.top && e.y <= (containerRect.top + containerRect.height))) {

              // Don't animate lights if nav menu overlay is showing
              var menuOverlay = document.getElementById("menu-overlay");
              if (getComputedStyle(menuOverlay, null).display != "none") {
                return;
              }

              _this3.ray.x = e.x - containerLeftOffset;
              _this3.ray.y = e.y - containerTopOffset + window.scrollY;
            }
        });
    }
